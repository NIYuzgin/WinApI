...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	if (hwnd == NULL)
	{
		MessageBox(NULL, "WindowsCreation failed", NULL, MB_OK | MB_ICONERROR);
		return 0;
	}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		(
			...
			"Edit",	// Class
			"0",	// Title
			WS_CHILD | WS_VISIBLE | WS_BORDER | ES_RIGHT,	// Style
			10, 10,		// Position
			g_i_DISPLAY_WIDTH, g_i_DISPLAY_HEIGHT,	// Size
			hwnd,		// Parent
			(HMENU)IDC_DISPLAY,
	...
	{
		HDC hdc = (HDC)wParam; // С сообщением WM_CTLCOLOREDIT в 'wParam' принимается HDC элемента EditControl
		SetBkMode(hdc, TRANSPARENT); // Делаем фон hEdit непрзрачным
	...
	case WM_KEYDOWN:
	{
		...
		{
		case VK_OEM_PLUS:
		case VK_ADD: SendMessage(GetDlgItem(hwnd, IDC_BUTTON_PLUS), BM_SETSTATE, TRUE, 0);
			break;
		case VK_OEM_MINUS:
		case VK_SUBTRACT: SendMessage(GetDlgItem(hwnd, IDC_BUTTON_MINUS), BM_SETSTATE, TRUE, 0);
			break;

		case VK_MULTIPLY: SendMessage(GetDlgItem(hwnd, IDC_BUTTON_ASTER), BM_SETSTATE, TRUE, 0);
			break;
		case VK_OEM_2:
		case VK_DIVIDE: SendMessage(GetDlgItem(hwnd, IDC_BUTTON_SLASH), BM_SETSTATE, TRUE, 0);
			break;
		case VK_OEM_PERIOD:
		case VK_DECIMAL: SendMessage(GetDlgItem(hwnd, IDC_BUTTON_POINT), BM_SETSTATE, TRUE, 0);
			break;

		case VK_BACK: SendMessage(GetDlgItem(hwnd, IDC_BUTTON_BSP), BM_SETSTATE, TRUE, 0);
			break;
		case VK_ESCAPE: SendMessage(GetDlgItem(hwnd, IDC_BUTTON_CLR), BM_SETSTATE, TRUE, 0);
			break;
		case VK_RETURN: SendMessage(GetDlgItem(hwnd, IDC_BUTTON_EQUAL), BM_SETSTATE, TRUE, 0);
			break;
